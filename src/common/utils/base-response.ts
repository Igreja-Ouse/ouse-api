...
export class BaseResponse<T = any> {
  ...
  private constructor(success: boolean, message: string, data?: T, statusCode?: number) {
    ...
    this.data = data;
    this.statusCode = statusCode || (success ? HttpStatus.OK : HttpStatus.BAD_REQUEST);
  }

  static success<T>(message: string = 'Operação realizada com sucesso', data?: T): BaseResponse<T> {
    return new BaseResponse(true, message, data, HttpStatus.OK);
  }

  static created<T>(message: string = 'Recurso criado com sucesso', data?: T): BaseResponse<T> {
    return new BaseResponse(true, message, data, HttpStatus.CREATED);
  }

  static error(message: string = 'Ocorreu um erro ao processar sua solicitação'): BaseResponse {
  ...
  static serverError(message: string = 'Erro interno do servidor'): BaseResponse {
    return new BaseResponse(false, message, null, HttpStatus.INTERNAL_SERVER_ERROR);
  }
}