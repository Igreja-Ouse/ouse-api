...
export class UsersService {
  ...
  async getById(id: number): Promise<BaseResponse<UserResponseDto>> {
    ...
    if (!user) return BaseResponse.notFound('Usuário não encontrado!');

    const firebaseUser = await this.firebaseService.getUserByUid(user.uid);

    return BaseResponse.success(
      '',
      this.userMapper.toResponseDto(user, firebaseUser),
    );
  }
  ...
  async create(userDto: CreateUserDto): Promise<BaseResponse<UserResponseDto>> {
    ...
    if (!firebaseUser) throw new Error('Erro ao criar usuário!');
    ...
    return BaseResponse.success(
      ...
      this.userMapper.toResponseDto(resp, firebaseUser),
    );
  }
  ...
  async updateRoles(
    ...
    if (!usuario.uid) throw new BadRequestException('Usuário não possui UID!');
    ...
    if (currentRolesStr === newRolesStr) {
      return BaseResponse.success(
        'Nenhuma alteração de papel necessária!',
        this.userMapper.toResponseDto(usuario, firebaseUser),
      );
    }

    await this.firebaseService.setCustomUserClaims(usuario.uid, {
      roles: dto.roles,
    });

    const updatedFirebaseUser = await this.firebaseService.getUserByUid(
      usuario.uid,
    );

    return BaseResponse.success(
      'Papel atualizado com sucesso!',
      this.userMapper.toResponseDto(usuario, updatedFirebaseUser),
    );
  }
  ...
  async update(userDto: UpdateUserDto): Promise<BaseResponse<UserResponseDto>> {
    ...
    if (usuarioFirebase) {
      ...
      if (usuarioFirebase.firstName || usuarioFirebase.lastName) {
        const newFirstName =
          usuarioFirebase.firstName || existingUser.nome.split(' ')[0];
        const newLastName =
          usuarioFirebase.lastName ||
          existingUser.nome.split(' ').slice(1).join(' ');
    ...
    const [updatedUser] = await Promise.all([
      ...
      ...(usuarioFirebase?.roles
        ? [
            this.firebaseService.setCustomUserClaims(existingUser.uid, {
              roles: usuarioFirebase.roles,
            }),
          ]
        : []),
    ]);
    ...
    return BaseResponse.success(
      ...
      this.userMapper.toResponseDto(updatedUser, firebaseUser),
    );
  }
